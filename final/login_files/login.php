          ...
          else  //passwords match
          {
            if (existing_username($db, $username)) {
              ...
              if (addUser($db, $username, $encrypt_password)){
                $_SESSION['current_user'] = $_POST['username'];
                // when new user is created send user to fill out new acount info
                header('Location: login_new_user_info.php');
              }
            else {
...
?>
    ...
    <main>              
        <form action="" method="post">
          ...
          <?php
             if ($_SESSION['type'] == "new"){
                echo "<label for='password2' class='repassword_label'>Retype password</label>";
                //would normally make the input type="password", but want to see what we type
